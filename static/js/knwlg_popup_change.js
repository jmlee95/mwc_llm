...
function initKnowledgePopup() {
        ...
        try {
            ...
            switch(buttonId) {
                case 'knowledge1':
                    ...
                    break;
                case 'knowledge2':
                    title = 'Summary of 5G Mobile Plans';
                    tabContents = [
                        { name: 'Product Information', id: 'tab1' }
                    ];
                    contentsList = [
                ...
                button.addEventListener('click', async function(e) {
                    ...
                    if (targetContent) {
                        ...
                        if (buttonId === 'knowledge1') {
                            switch(tabId) {
                                case '1':
                                    contentPath = '/knwlgFile/01_mobile_plan/5G 슬림 요금제 ver.7 [24.03.22]`_eng.html';
                                    break;
                                case '2':
                                    contentPath = '/knwlgFile/01_mobile_plan/5G 슬림 요금제_이벤트_eng.html';
                                    break;
                                case '3':
                                    contentPath = '/knwlgFile/01_mobile_plan/5G 슬림 요금제_FAQ_eng.html';
                                    break;
                            }
    ...
    // 지식 버튼 클릭 시 active 클래스 추가
    function addKnowledgeButtonClickEvent(button) {
        button.addEventListener('click', function() {
            // 기존 active 클래스 제거
            document.querySelectorAll('.btn_knowledge').forEach(btn => {
    ...
    // 지식 버튼에 이벤트 리스너 추가
    if (knowledgeBtn1) addKnowledgeButtonClickEvent(knowledgeBtn1);
    if (knowledgeBtn2) addKnowledgeButtonClickEvent(knowledgeBtn2);
}
